...
So, first, let's go back to the initial coordinates, that I still start from, for simplicity.
...
To convert from cartesian to polar coords, we compute
...
float lander_polar_coords[24][2];

for(size_t c = 0 ; c < 24 ; ++c) {
    const float x = lander_coords[c][0];
    const float y = lander_coords[c][1];

    lander_polar_coords[c][0] = sqrt(x * x + y * y);
    lander_polar_coords[c][1] = atan2f(y, x);
}
```
